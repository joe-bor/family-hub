...
Implementation merged via **FE PR #194** (2026-06-10); CI lint/unit/E2E green. All ten detailed acceptance criteria below are satisfied in code. Two behaviors depend on on-device verification (jsdom/Playwright cannot assert them) and are noted **on-device smoke pending**: swipe-left-to-close on the side sheet, and focused inputs sitting above the soft keyboard. The "More" sheet rides the later vaul `MobileSheet` rebuild (FE PR #198, story [expandable-bottom-sheet](expandable-bottom-sheet.md)) and now opens at half height — superseding this spec's hand-rolled-sheet description and its "no vaul migration" non-goal.
