...
- BE implementation is in PR [family-hub-api#69](https://github.com/joe-bor/family-hub-api/pull/69) (open, not yet merged/released; reviewed and hardened — deterministic read-order tie-break, named `MAX_BULK_ITEMS` + service guard, non-empty-append test; full `./mvnw test` green, 544 tests). Once merged, publish a `family-hub-api` release so FE #277's released-contract E2E can consume the published semver. Record further PR links here as they land.
