...
- **Preferences surface** — `src/components/settings/preferences-sheet.tsx`: a `ResponsiveFormDialog` with a "Family Timezone" section and a disabled "Coming Soon" stub section. Rendered from `src/components/shared/sidebar-menu.tsx:183`. This is where the haptics toggles go.
